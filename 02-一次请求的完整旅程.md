...
> `stream` 字段默认是 `False`（[`api_server.py:78`](../../python/minisgl/server/api_server.py)）。[#125](https://github.com/sgl-project/mini-sglang/pull/125) 之后 `stream=false` 也被支持：服务端会把所有增量拼成一整段，最后返回**一个** `chat.completion` JSON（而不是 SSE 流）。下面 5 步讲的是 `stream=true` 路径，非流式只是在第 5 步换了个收尾方式（见后文）。

进入服务端的入口是 [`api_server.v1_completions`](../../python/minisgl/server/api_server.py:255-310)。整段函数本身只做 5 件事：
...
5. 根据 `req.stream` 分两种收尾方式：
   - `stream=true`：返回 `StreamingResponse(stream_chat_completions(uid))`，由 FastAPI 调用这个 async generator 异步地边读边吐 SSE。
   - `stream=false`：直接 `async for ack in state.wait_for_ack(uid)` 把所有增量累加成 `full_content`，遇到 `ack.finished` 跳出，最后 `return` 一个完整的 `chat.completion` JSON。注意此时 handler 会**一直 await 到生成结束**才返回（区别于流式那种立刻让出控制权）。两条路径复用的是同一个 `wait_for_ack`（见 2.7），只是一个 yield 给 SSE、一个在本地拼接。
...
> **非流式（`stream=false`）的收尾**：不经过 `stream_chat_completions` 这层 SSE 包装，`v1_completions` 直接 `async for ack in wait_for_ack(uid)` 把每个 `ack.incremental_output` 拼进 `full_content`，`ack.finished` 后一次性 `return` 一个 `{"object": "chat.completion", "choices": [{"message": {...}}]}`。代价是客户端要等整段生成完才拿到响应；好处是兼容只认单条 JSON、不会解析 SSE 的 OpenAI SDK 调用。目前 `usage` 里的 token 数还是占位的 0（[`api_server.py:305-309`](../../python/minisgl/server/api_server.py)）。
